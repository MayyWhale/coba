...
const jurusan = (utils) => {
  ...
  const addButton = $("#addJurusanButton");

  const onJurusanModalButtonClicked = async () => {
    utils.myModal.title.text(utils.myModal.id ? "Edit Jurusan" : "Tambah Jurusan");
    utils.myModal.body.html(`<loader-spinner></loader-spinner>`);
    utils.myModal.modal.toggle();
    try {
      const result = await DataSource.formJurusan(utils.myModal.id ?? null);
